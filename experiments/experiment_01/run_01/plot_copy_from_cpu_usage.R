...
copy_from_times <- read.table("copyFromTimes_1339006816928_RFriendly")
...
png("time_vs_cpuUsage_vs_copyFrom.png")

p <- ggplot(data=power_and_cpu_use_data, aes(y=X.usr,x=timestamp, fill=category, group=group)) + geom_area() 
p <- p + coord_cartesian(ylim=c(0,100)) 
p <- p + xlab("Time") + ylab("CPU usage (%)")
