...
public final class JEEventScheduler {
    ...
    public void queue_event(JEEvent aNewEvent) {
		...
		} else if ( EventList.lastElement().getTheScheduledTime().isEarlierThan(anEventTime)) {
			EventList.addElement(aNewEvent);
		} else {
		    
			int queue_pos;
		    
		    for (queue_pos = queue_size - 1; ( (queue_pos > 0) & anEventTime.isEarlierThan(EventList.elementAt(queue_pos).getTheScheduledTime())); queue_pos--) {
			/* empty */
		    }
		    
		    if (++queue_pos == 1 & anEventTime.isEarlierThan(EventList.elementAt(0).getTheScheduledTime())) {
		    	queue_pos--;
		    }
