	...
	public static List<Integer> random(int samplesCount, int n) {
		...
		if (samplesCount < 0)
			throw new IllegalArgumentException(
					"samplesCount cannot be negative: " + samplesCount);

		// n is exclusive, so <=
		if ((n <= 0) || (n <= samplesCount))
			throw new IllegalArgumentException("ceil value must be positive "
					+ "and greate than samples count: n <" + n
