...
public class ReplicationGroup implements Comparable<ReplicationGroup> {
	 ...
	 * @param replicationLevel TODO
	 * @param primaryDataServer
	 * @param secondariesReplicas
	 */
	public ReplicationGroup(String fileName, long initialFileSize, int replicationLevel, final DataServer primaryDataServer, 
			final List<DataServer> secondariesReplicas) {
		
		//FIXME secs should be a set, no duplicates

		if (fileName == null) throw new IllegalArgumentException();
		if (primaryDataServer == null) throw new IllegalArgumentException();
		if (secondariesReplicas == null) throw new IllegalArgumentException();
		...
		this.theUniqueEventScheduler = emulatorControl.getTheUniqueEventScheduler();
		this.replicationDelay = new JETime(emulatorControl.replicationDelayMillis);
	...
	public void migrate(DataServer newDataServer) {
		...
		if (hasReplica(newDataServer)) throw new RuntimeException();
	...
	private void promoteSecondary(DataServer secondary) {
		...
		Aggregator.getInstance().report("promoting "+getTime() + " " + emulatorControl.getTheUniqueEventScheduler().now()+ " "+fileName);
	...
	private void scheduleMigration(DataServer dstDataServer) {
		...
		long delta = (long) (2 + Math.ceil( (primary.getFileSize(getFileName()) * 8 ) / sustainedThroughtput_bits_per_sec)); //2 millisecond by control messages
		JETime time = emulatorControl.getTheUniqueEventScheduler().now().plus(new JETime(delta * 1000));
		MigrateEvent migrateEvent = new MigrateEvent(getFileName(), dstDataServer, emulatorControl.getMetadataServer(), time);
	 ...
	 * @param client
	 */
	public void writeFile(String fileName, long offset, long length, DDGClient client) {
		
		setTouchTimeStamp(EmulatorControl.getInstance().getTheUniqueEventScheduler().now());
		
		checkDataMigrationAndPromotion(client);
		reportDataOperation(client.getMachine(), primary, DataOperation.WRITE, length);
	...
	private void writeFile(DataServer dataServer, String fileName, long offset, long length, DDGClient client) {
		
		if (!dataServer.containsFile(fileName)) {
			throw new RuntimeException("File " + fileName + " does not exist on data server: " + dataServer.getId());
		}
	...
	private void checkDataMigrationAndPromotion(DDGClient client) {
		...
		DataServer clientDS = client.getMachine().getDeployedDataServers().iterator().next();

		if ( ! hasReplica(clientDS) && emulatorControl.datamigration) {
			if (!processingMigration) {
				processingMigration = true;
				scheduleMigration(client.getMachine().getDeployedDataServers().iterator().next());
			}
	...
	private boolean isUptoDate() {
		return getTime().plus(replicationDelay).isEarlierThan(emulatorControl.getTheUniqueEventScheduler().now());
	}
	...
	public void read(String fileName, long offset, long length, DDGClient client) {
		checkDataMigrationAndPromotion(client);
		reportDataOperation(client.getMachine(), primary, DataOperation.READ, length);
	}
	
	private void reportDataOperation(Machine machine, DataServer dataServer, DataOperation dataOperation, long length) {
		Aggregator.getInstance().reportDataOperation(fileName, dataOperation, length, machine.isDeployed(dataServer), machine.toString());
	}
	...
	private JETime getTime() {
		return (getTouchTimeStamp() != null) ? getTouchTimeStamp() : creationTime; 
	}
	...
	public String toString() {
		...
		String header = getFileName()+"#"+getDsString(primary, getFileName());
