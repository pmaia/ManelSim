...
public class Aggregator {
	...
	public enum DataOperation {READ, WRITE};
	...
	private Aggregator() {
		...
		//	(0, 0, 0, 0) # (totalbytes_co-located, totalbytes, totalops_co-located, totalops)
	 ...
	 * @param machine TODO
	 */
	public void reportDataOperation(String filename, DataOperation dataOperation, long length, boolean wasLocal, String machine) {
		
		//(0, 0, 0, 0) # (totalbytes_co-located, total bytes, totalops_co-located, total ops)
	...
	private void updateTrace(long[] trace, boolean wasLocal, long length) {
		...
		//TAKE CARE USING THIS METHOD, DO NOT CALL IT ON REPLICA MANUTENTION DATA TRANSFER, IN DOING SO IT WILL BE UNBALANCED
		if ( wasLocal) {
			trace[0] += length;
			trace[2] += 1;
		}
	...
	public void reportlogin(DDGClient client, long now) {
		DataServer dataServer = client.getMachine().getDeployedDataServers().get(0);
		loginlogs.add(dataServer + "\t" + dataServer.getAvailableDiskSize() + "\t" + now);
	}
	...
	public String toString() {
		...
		for (Entry<String, Map<DataOperation, long[]>> fileData : file2Data.entrySet()) {
			buffer.append(fileData.getKey() + "\t");
			appendToBuffer(buffer, fileData.getValue().get(DataOperation.READ), fileData.getValue().get(DataOperation.WRITE));
		}
